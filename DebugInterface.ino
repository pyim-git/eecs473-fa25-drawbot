...
volatile double Kp_rho   = 4.0;
volatile double Kp_alpha = 0.7;
volatile double Kp_theta = 12.0;
volatile double Kp = 300.0;
volatile double Ki = 0.01;
volatile double Kd = 20.0;
// --- WebSocket Functions ---
...
volatile int manual = 1;
int c = 0;
double setpointx[5] =     { 0.3,  0.3,    0,   0};
double setpointy[5] =     { 0.0,  0.3,    0.3,  0};
double setpointtheta[10] = {0.0, 0.0, M_PI/2, M_PI/2,0};
// double setpointx[10] =     {0.0, 0.0,  0.2,    0.2,   0.2, 0.0 , 0.0    , 0.0,    0.0, 0.2};
// double setpointy[10] =     {0.0, 0.0,  0.0,    0.2,   0.2, 0.2 , 0.2    , 0.4 ,    0.4,0.4};
// double setpointtheta[10] = {0.0, 0.0, M_PI/2, M_PI/2, M_PI,M_PI, M_PI/2 ,  M_PI/2, 0.0,0.0};
...
void processCommand(String cmd) {
  ...
  if (cmd.length() > 0) {
    ...
    else if (cmd.startsWith("RobotPos")) {
      int xIdx = cmd.indexOf("x=");
      int yIdx = cmd.indexOf("y=");
      
      if (xIdx >= 0) {
        int end = cmd.indexOf(' ', xIdx + 2);
        camX = cmd.substring(xIdx + 2, end == -1 ? cmd.length() : end).toFloat();
      }
      if (yIdx >= 0) {
        int end = cmd.indexOf(' ', yIdx + 2);
        camY = cmd.substring(yIdx + 2, end == -1 ? cmd.length() : end).toFloat();
      }
...
void navigationTask(void *pv) {
    ...
    // Tolerances
    const double GOAL_TOLERANCE = 0.03;           // Position tolerance (m)
    const double ANGLE_TOLERANCE = 0.02;          // Increased from 0.01 (~2.86° instead of 0.57°)
    const double SEVERE_DRIFT_THRESHOLD = 0.5;   // Increased from 0.52 (~60° instead of 30°)
    const double DRIFT_WARNING_THRESHOLD = 0.35;  // Increased from 0.087 (~20° instead of 5°)
    
    // Velocity limits
    const double MAX_LINEAR_VEL = 10;            // m/s
    const double MAX_ANGULAR_VEL = 10;           // rad/s
    const double MAX_WHEEL_SPEED = 10;           // rev/s
...
//use ws.textAll instead of Serial.print