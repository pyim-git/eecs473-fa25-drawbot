...
int remapPID2(double pidOutput, bool rotatingInPlace) {
...
void encoderTask(void *pv) {
  ...
  for (;;) {
    ...
    // Serial.print(myPosition.x - x);
    // Serial.println("Y Error");
    // Serial.print(myPosition.y - y);
    // Serial.println("Theta Error");
    // Serial.print(myPosition.h - theta);


    // Calculate velocities with low-pass filter ty - y reduce noise
...
void navigationTask(void *pv) {
    ...
    const double goal_tolerance  = 0.01;  // meters
    const double angle_tolerance = 0.01;  // radians (~2 degrees)
...
void setup() {
  ...
  if (!myOtos.begin()) {
    ...
    Serial.println("System halted.");
    while(1) delay(1000);  // Halt here instead of crashing
  }
  Serial.println("Robot Navigation Starting...");

  myOtos.calibrateImu();
  myOtos.resetTracking();
  myOtos.setAngularScalar(0.9985437903); // -> 5.6
